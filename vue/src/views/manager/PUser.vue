    ...
    <div class="main-body-content">
      <div style="width: 50%; padding: 10px 0">
        <el-form :model="form" label-position="right" label-width="100px" style="padding-right: 40px">
          ...
          <!-- disabled将这个输入框变为不可更改，更改的话需要通过充值进行 -->
...
export default {
  ...
  methods: {
    ...
    // 充值处理
    recharge() {
      let data = {id: this.form.id, account: parseFloat(this.form.account) + parseFloat(this.account)}; // 充值后余额
      request.put("/user", data).then(res => {
        if (res.code === '0') {
          this.$notify.success('充值成功');
          this.dialogVisible = false;
          this.getPerson(); // 充值成功后重新获取用户信息
        } else {
          this.$notify.error(res.msg);
        }
      })
